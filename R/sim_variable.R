...
sim_cX <- function(n, family, pars, dat, quantiles){
  ...
  if (family[[1]] == "logistic" || family[[1]] == 9){
    beta <- pars[[1]][[1]]
    phi <- pars[[1]][[2]]
  ...
  else if (family[[1]] == "normal" || family[[1]] == 1){
    beta <- pars[[1]][[1]]
    phi <- pars[[1]][[2]]
    t_star <- rnorm(n, beta,phi)
    ut_star <- pnorm(t_star,beta,phi)
    threshold <- 0 # T*=0 as threshold. T=1 if T*>0; T=0 if T*<=0
    u_threshold <- pnorm(threshold,beta,phi)
  }
  else{
    stop(paste0(family[[1]]),"does not support for X")
  }
  ...
  for (i in 1:dimZ){
    if (family[[2]][[1]][[i]] == 1){
      ...
      p[,i+1] <- pnorm(qnorm(p[,i])*sqrt(1-params[[1]][[i]]^2)+params[[1]][[i]]*qnorm(quantiles[,i]))
      #ps[,i+1] <- 1-pnorm((qnorm((1-ps[,i]))  - params[[i]] *qnorm( quantiles[,i])) / sqrt(1 - params[[i]]^2))
      ps[,i+1] <- 1-pnorm(qnorm(1-ps[,i])*sqrt(1-params[[1]][[i]]^2)+params[[1]][[i]]*qnorm(quantiles[,i]))
      ...
      scaled_qZs[,i] <- apply(input_data, 1, function(row) {
        integrand <- function(uz) {
          1 - pnorm((qnorm((1 - row[1])) - params[[1]][[i]] * qnorm(uz)) / sqrt(1 - params[[1]][[i]]^2))
        }
        integrate(integrand, lower = 0, upper = row[2])$value / row[1]
      })
    }
    ## (to be completed)
    else if (family[[2]][[1]][[i]] == 2) {
      # Y <- sqrt(phi)*qt(U, df=pars$par2) + eta
      p[,i+1] <- cVCopula(data.frame(quantiles[,i],p[,i]), copula = tCopula, param = params[[1]][[i]], par2=params[[2]][[i]], inverse=FALSE)
      ps[,i+1] <- 1-cVCopula(data.frame(quantiles[,i],1-ps[,i]), copula = tCopula, param = params[[1]][[i]], par2=params[[2]][[i]], inverse=FALSE)
    }
    else if (family[[2]][[1]][[i]] == 3) {
      p[,i+1] <- cVCopula(data.frame(quantiles[,i],p[,i]), copula = claytonCopula, param = params[[1]][[i]], inverse=FALSE)
      ps[,i+1] <- 1-cVCopula(data.frame(quantiles[,i],1-ps[,i]), copula = claytonCopula, param = params[[1]][[i]], inverse=FALSE)
    }
    else if (family[[2]][[1]][[i]] == 4) {
      p[,i+1] <- cVCopula(data.frame(quantiles[,i],p[,i]), copula = gumbelCopula, param = params[[1]][[i]], inverse=FALSE)
      ps[,i+1] <- 1-cVCopula(data.frame(quantiles[,i],1-ps[,i]), copula = gumbelCopula, param = params[[1]][[i]], inverse=FALSE)
      
    }
    else if (family[[2]][[1]][[i]] == 5) {
      p[,i+1] <- cVCopula(data.frame(quantiles[,i],p[,i]), copula = frankCopula, param = params[[1]][[i]], inverse=FALSE)
      ps[,i+1] <- 1-cVCopula(data.frame(quantiles[,i],1-ps[,i]), copula = frankCopula, param = params[[1]][[i]], inverse=FALSE)
    }
    else if (family[[2]][[1]][[i]] == 6) {
      p[,i+1] <- cVCopula(data.frame(quantiles[,i],p[,i]), copula = joeCopula, param = params[[1]][[i]], inverse=FALSE)
      ps[,i+1] <- 1-cVCopula(data.frame(quantiles[,i],1-ps[,i]), copula = joeCopula, param = params[[1]][[i]], inverse=FALSE)
    }
